...
async def show_marathons_menu(message: types.Message):
    ...
    await message.answer(
        ...
        "Доступ к ним предоставляется после оплаты.\n"
        "Выберите интересующий марафон или откройте доступ:",
        reply_markup=keyboard
    )

@router.message(lambda msg: msg.text in {"🎯 Подготовка к садику", "✏ Подготовка к школе"})
async def locked_content_handler(message: types.Message):
    await message.answer(
...
async def open_access_handler(message: types.Message):
    await message.answer(
        "💳 Чтобы получить полный доступ к марафонам, перейдите по ссылке для оплаты: [Ваша ссылка на оплату]\n\n"
